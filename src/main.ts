...
<a href="expressive-marks-attached-to-notes#dynamics">Dynamics</a>
        ...
        const tableRows = Array.from(document.querySelectorAll("table.index-ky > tbody > tr"));

        const tableRowsWithThreeTds = tableRows.filter((tr: HTMLTableRowElement) => {
            const children = Array.from(tr.children);
            const tds = children.filter((c) => c.nodeName === `TD`);
            return tds.length === 3;
        })

        const codeTokenElems = tableRowsWithThreeTds.map((tr: HTMLTableRowElement) => {
            const children: HTMLTableCellElement[] = <HTMLTableCellElement[]>Array.from(tr.children);
            const codeElement: Element = children[1].firstElementChild.firstElementChild;
            const descElement: HTMLLinkElement = <HTMLLinkElement>children[2].firstElementChild;
...
(async () => {
    ...
    const snippetElems: SnippetElement[] =
        elems.map((e: CodeTokenElement) => {
